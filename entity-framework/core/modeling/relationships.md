...
---
# <a name="relationships"></a>关联关系

关联关系定义了两个实体之间是如何相互关联的。在关系数据库中，这是用外键约束表示的。

> [!NOTE]  
> 这篇文章中的大部分示例使用“一对多”关系来演示概念。 有关“一对一”和“多对多”关系的示例，请参阅本文末尾部分的[其他关系模式](#other-relationship-patterns)。

## <a name="definition-of-terms"></a>术语的定义

有一些用于描述关联关系的术语

* **相关实体：** 这是包含外键特性的实体。有时也把这个实体称为关联关系中的“孩子（child）”。

* **主体实体：** 这是包含主键或备用键特性的实体。有时也把这个实体称为关联关系中的“父亲（parent）”。

* **外键：** 外键是相关实体中的一个特性，这个特性用来存储相关联的实体的主体键的值。

* **主体键：** 这个特性唯一地标识一个主体实体。主体键可能是主键或备用键。
...
按照约定，当在一个类型上发现导航特性时，关联关系会被创建。如果一个特性指向的类型不能被当前的数据库提供程序映射为标量（scalar ）类型，这个特性被认为是导航特性。
...
关联关系中最常用的模式是，在关系的两端都定义导航特性，并且把外键特性定义在相关实体的类中。
...
## <a name="other-relationship-patterns"></a>其他关联关系模式

### <a name="one-to-one"></a>一对一

“一对一”关系的两端都拥有一个引用导航特性。“一对一”关系遵循和“一对多”关系相同的约定，但在外键特性上引入了一个唯一索引，以确保每一个主体（实体）上只有一个关联的相关（实体）。
